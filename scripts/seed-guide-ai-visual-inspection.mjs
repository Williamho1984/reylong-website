...
//   *** EXTERNAL EVIDENCE — CORRECTED 2026-08-13. READ BEFORE RESTORING ANY OLD NUMBER ***
//   - The ONLY external study citable here:
...
//       Verbatim: "Inspectors correctly rejected 85% of defective items and incorrectly
//       rejected 35% of acceptable parts."
...
//       Always say it is precision manufactured parts, NOT packaging — it is an analogy.
//       Always give BOTH numbers; the 35% false-reject half is the more useful one.
//   - REMOVED, MUST NOT RETURN (all were live and wrong):
//       * "inspectors miss 20–30% (Sandia research)" — NOT a Sandia finding. That came from
//         SAND2012-8590 (OSTI 1055636), a literature REVIEW of 212 third-party papers.
//         Never link osti.gov/biblio/1055636 as evidence of a finding.
//       * "attention degrades after ~2 h" — in NEITHER document. No source. Also removed from
//         the softer phrasings ("a tired eye starts missing two hours into a shift").
//       * "AI false-reject <1% vs manual 10–20%" — no named source. Deleted.
//   - Rey Long numbers are SPECIFICATIONS, not achieved customer results. Write "specified for"
//     / "design target", never "deployment target" or "in our implementations".
//
...
const faq = [
  {
    ...
    a_en: 'Three families. Printing defects: registration drift, ink skip, blur and smearing. Material defects: broken warp or weft filaments, weave holes and surface fuzz. Processing defects: skipped stitches, cut-point offset and uneven hems. The common thread is that all of these are visible, recurring faults — exactly the kind a camera watching every bag at line speed is good at, and exactly the kind a human eye stops catching reliably once it has been watching the same web for a long stretch.',
    q_es: '¿Qué defectos puede detectar la visión por IA en sacos tejidos?',
    a_es: 'Tres familias. Defectos de impresión: desviación de registro, falta de tinta, borrosidad y manchado. Defectos de material: filamentos de urdimbre o trama rotos, agujeros en el tramado y pelusa superficial. Defectos de proceso: puntadas saltadas, desplazamiento del punto de corte y dobladillos irregulares. El hilo común es que todos son fallos visibles y recurrentes — exactamente el tipo de fallo que una cámara que observa cada saco a velocidad de línea detecta bien, y exactamente el que un ojo humano deja de captar de forma fiable tras un buen rato mirando la misma banda.',
  },
  {
...
<p>Two numbers matter, and they pull against each other. <strong>Recall</strong> is the share of real defects the system catches; Rey Long specifies 95%+ recall as a design target, and the honest qualifier is that the achieved figure is application-dependent &mdash; it varies with fabric, artwork, lighting and which defect classes you care about, which is why it is established during commissioning on your product rather than quoted from a datasheet. <strong>Precision</strong> is the share of alerts that are real; its inverse is the false-reject rate. False rejects matter more than they sound, because an inspection system that cries wolf trains operators to ignore it, at which point its recall is irrelevant &mdash; and unlike a human threshold, a model's threshold is written down, so you can review it and move it.</p>
<p>For comparison, on the human side: a <a href="https://www.sandia.gov/research/publications/details/visual-inspection-reliability-for-precision-manufactured-parts-2015-12-01/">Sandia National Laboratories study of visual inspection reliability</a> found inspectors correctly rejected 85% of defective items &mdash; and incorrectly rejected 35% of acceptable ones. That work examined precision manufactured parts rather than packaging, so it is an analogy for a bag line rather than a measurement of one; what transfers is that manual inspection loses on both sides at once. The camera's advantage is not superhuman perception on any single bag &mdash; it is that bag ten thousand gets the same inspection as bag one. We compare the two approaches in detail in <a href="/news/manual-vs-ai-inspection-woven-bag-lines">the manual-vs-AI guide</a>.</p>
...
<p>It repositions it rather than replacing it. In a Sandia National Laboratories study of visual inspection reliability, inspectors correctly rejected 85% of defective items and incorrectly rejected 35% of acceptable ones &mdash; losing on both sides at once. That study looked at precision manufactured parts rather than packaging, so it is an analogy for a bag line and not a measurement of one; but the camera takes over the task humans are demonstrably bad at: watching every bag, all shift, without fatigue. What stays human is judgement: setting the quality standard, deciding what to do when the system alerts, and the subjective calls a camera cannot make. In practice this shifts staffing rather than eliminating it &mdash; one operator can supervise up to four machines instead of two.</p>
...
<p>Importan dos n&uacute;meros, y tiran en direcciones opuestas. La <strong>exhaustividad</strong> (recall) es la proporci&oacute;n de defectos reales que el sistema detecta; Rey Long especifica superar el 95% como objetivo de dise&ntilde;o, y el matiz honesto es que la cifra alcanzada depende de la aplicaci&oacute;n &mdash; var&iacute;a con el tejido, el arte, la iluminaci&oacute;n y las clases de defecto que le importen, y por eso se establece durante la puesta en marcha sobre su producto y no se cita de una ficha t&eacute;cnica. La <strong>precisi&oacute;n</strong> es la proporci&oacute;n de alertas que son reales; su inversa es la tasa de rechazos falsos. Los rechazos falsos importan m&aacute;s de lo que parece, porque un sistema de inspecci&oacute;n que grita "lobo" entrena a los operarios a ignorarlo, y en ese punto su exhaustividad es irrelevante &mdash; y a diferencia de un umbral humano, el umbral de un modelo est&aacute; escrito, as&iacute; que se puede revisar y mover.</p>
<p>Como comparaci&oacute;n, del lado humano: un <a href="https://www.sandia.gov/research/publications/details/visual-inspection-reliability-for-precision-manufactured-parts-2015-12-01/">estudio de Sandia National Laboratories sobre fiabilidad de la inspecci&oacute;n visual</a> hall&oacute; que los inspectores rechazaron correctamente el 85% de las piezas defectuosas &mdash; y por error el 35% de las aceptables. Ese trabajo examin&oacute; piezas de manufactura de precisi&oacute;n y no envases, as&iacute; que es una analog&iacute;a para una l&iacute;nea de sacos y no una medici&oacute;n de ella; lo que se traslada es que la inspecci&oacute;n manual pierde por los dos lados a la vez. La ventaja de la c&aacute;mara no es una percepci&oacute;n sobrehumana en un saco concreto &mdash; es que el saco diez mil recibe la misma inspecci&oacute;n que el saco uno. Comparamos ambos enfoques en detalle en <a href="/es/news/manual-vs-ai-inspection-woven-bag-lines">la gu&iacute;a manual vs IA</a>.</p>
...
<p>Lo reposiciona m&aacute;s que sustituirlo. En un estudio de Sandia National Laboratories sobre fiabilidad de la inspecci&oacute;n visual, los inspectores rechazaron correctamente el 85% de las piezas defectuosas y por error el 35% de las buenas &mdash; perdiendo por los dos lados a la vez. Ese estudio trata de piezas de manufactura de precisi&oacute;n y no de envases, as&iacute; que es una analog&iacute;a para una l&iacute;nea de sacos y no una medici&oacute;n de ella; pero la c&aacute;mara asume la tarea en la que los humanos son demostrablemente malos: vigilar cada saco, todo el turno, sin fatiga. Lo que sigue siendo humano es el criterio: fijar el est&aacute;ndar de calidad, decidir qu&eacute; hacer cuando el sistema alerta y los juicios subjetivos que una c&aacute;mara no puede hacer. En la pr&aacute;ctica esto redistribuye el personal en lugar de eliminarlo &mdash; un operario puede supervisar hasta cuatro m&aacute;quinas en lugar de dos.</p>
